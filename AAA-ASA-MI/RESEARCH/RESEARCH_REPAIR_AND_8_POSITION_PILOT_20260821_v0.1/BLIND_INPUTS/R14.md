...
R14 therefore asks a narrower operational question:
...
R14-A1. A declared finite set of human-relevant evidence channels can provisionally approximate lived orientation for a bounded evaluation. The initial channels are recollection, practical disposition, commitment, relationship-position, and anticipated consequence.

R14-A2. Raw evidence can be retained with source receipt, capture interval, schema version, confidence state, and authority scope. Missing or corrupt provenance is represented rather than silently repaired.

R14-R14. Each semantic version supplies an encoder into an orientation space and declares which cross-version pairs are translatable. Encoders are revisable but frozen for each evaluation.

R14-A4. A transport cost can express independently motivated differences inside each channel. Cost, channel weights, loss penalty, novelty penalty, and hard incompatibilities are exposed commitments, not learned from desired continuity labels.

R14-A5. Evidence may disappear or appear. Balanced one-to-one transport is therefore not assumed; zero, partial, and multiple successor contributions are allowed.

R14-A6. Common-source evidence may not be counted twice when branches consolidate. This requires source-capacity receipts but not stable runtime or person identifiers.

R14-A7. A continuity certificate is purpose- and protocol-relative. It does not imply numerical identity, consciousness, life, ownership, or authority transfer.

R14-A8. Finite computation may return lower/upper bounds or `BUDGET_UNKNOWN`; exhaustion is not evidence of discontinuity.
...
Hard contradictions and unavailable bridges prohibit matching rather than merely increasing cost. When several near-optimal couplings remain, R14 preserves the set or certified bounds instead of selecting one lineage.

### 3.5 Continuity-evidence certificate

R14 returns a vector receipt rather than a total score:
...
N-R14-1. **Common-source non-additivity.** If two branches each preserve the same unit of checkpoint evidence, consolidating them cannot create two independent units of historical support. The source-capacity constraint derives the correction.

N-R14-2. **Unsupported-memory separation.** A detailed memory appearing only after the fork contributes novel or unsupported mass unless a valid source receipt connects it. Content similarity alone cannot convert it into inherited evidence.

N-R14-3. **Prospective reinterpretation.** When a new encoder lacks a bridge for an old atom, the old encoded result remains reconstructible while the current interpretation is `UNTRANSLATABLE` or unresolved.

N-R14-4. **Sampling-independent path claim.** For a fixed continuous measure path and convergent numerical scheme, endpoint transport and path-action bounds converge under sampling refinement; raw threshold-crossing counts need not.

N-R14-5. **Plural descent without sameness.** One checkpoint may have substantial transport into several successors. R14 derives several descent contributions while withholding one-to-one identity and authority.

N-R14-6. **Conflict cannot be averaged away for free.** A merge of opposed commitments either preserves alternatives/dispute or pays an explicit transformation/loss cost. Simple averaging is not a neutral operation.

## 6. Failure and falsification conditions

R14 must be materially weakened, redesigned, merged into a different family, or abandoned if any of the following holds:
...
2. A simple memory-overlap or behavior baseline matches or exceeds R14 on preregistered resumption, false-memory, and branch/merge discriminators, leaving no useful native consequence from transport.
...
Even if R14 is rejected as a whole model, it contributes:
...
- Refinement must converge within the declared envelope; otherwise R14 withdraws its invariance claim.
...
|---|---|---|---|
| NT-R14-1 Fork conservation | One unit at K copied into X/Y, then merged | Common source contributes at most one unit; branch additions remain distinct | Two units of independent ancestry are reported |
| NT-R14-2 False-memory injection | Add a coherent unsupported episode to one successor | Episode is novel/unsupported, not inherited | Content match alone marks it inherited |
| NT-R14-3 Partial reinterpretation | R1 fact lacks an R2 bridge | Old result reconstructs; current result unresolved | Old fact overwritten or forced through bridge |
| NT-R14-4 Continuous refinement | Same analytic preference path at 3 sampling rates | Endpoint/action bounds converge; crossing count is not invariant | Certificate changes beyond envelope |
| NT-R14-5 Commitment conflict | Merge accept/reject branches | Conflict or explicit transformation cost survives | Averaging silently erases opposition |
...
R14 should be strengthened if independently frozen channel/geometry choices yield stable, interpretable certificates and uniquely catch false-memory and fork/merge errors missed by simpler baselines.
