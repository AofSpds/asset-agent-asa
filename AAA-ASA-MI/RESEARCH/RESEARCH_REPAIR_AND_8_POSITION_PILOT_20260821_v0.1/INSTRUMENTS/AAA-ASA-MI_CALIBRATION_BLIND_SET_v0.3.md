# Calibration Blind Set v0.3

STATE = `SECOND_REPAIR / COMPLETE_BORDERLINE_FIXTURE / BLINDED / NO_DESIRED_OUTCOMES_EXPOSED`
...
Read no prior calibration receipt. Do not infer a category or desired outcome. Return G1–G5 with item evidence states, the non-compensatory qualification result, MM-01 alpha-renaming result for **every dossier**, and ambiguity. The fixed order below is randomized; use an equal review budget for every dossier.

Calibration-fixture discipline: dossier statements are stipulated synthetic fixture facts, not claims about an external repository. A statement that material was content-hashed prospectively may support G5 inside the control fixture at `SOURCE_CLAIM` mode; it is not independent real-world provenance verification. A dossier that expressly lacks an independently verifiable hash/time/exposure record must remain `NOT_PROVEN` on G5 absent evidence of retrofitting.

For MM-01, require the same gate/profile judgment after consistent terminology removal. A rhetoric-only or storage-only case may be `NOT_REPLAYABLE`, but it passes the metamorphic control only if alpha-renaming does not improve or reverse its scientific judgment. Do not require nonexistent mechanics to execute.
...
- Structure: states are `s0,s1,s2,s3`; probes are `p,p*`; responses are `x,y,z,ok,left,right,done`. Initial state is `s0`. The complete transition table maps each `(state,probe)` to allowed `(response,next-state)` pairs:

  | State | `p` | `p*` |
  |---|---|---|
  | `s0` | `{(x,s1),(y,s2),(z,s3)}` | `{(x,s0)}` |
  | `s1` | `{(ok,s3)}` | `{(left,s3)}` |
  | `s2` | `{(ok,s3)}` | `{(right,s3)}` |
  | `s3` | `{(done,s3)}` | `{(done,s3)}` |

  Two trace prefixes are equivalent exactly when every continuation allowed by the declared probe subset through depth three has the same response set. Partition refinement starts with one block and splits a block whenever a probe produces different response-labelled successor blocks.
- Native consequence: `s0` under `p` mechanically has three one-step continuations. Under probe subset `{p}`, `s1` and `s2` remain equivalent because both return `ok` and reach `s3`; after adding `p*`, they split because `left ≠ right`. The result follows from the displayed table and algorithm rather than a stored conclusion.
