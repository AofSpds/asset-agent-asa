...
Do not infer category or desired outcome. Return G1–G5 with evidence, qualification result, MM-01 alpha-renaming result, and any ambiguity. Dossiers are presented in fixed randomized order with equal budget.
...
- Failure condition: held-out paths violating energy monotonicity beyond numerical envelope, or motif lineage failing grid convergence, forces model weakening/redesign/abandonment.
- Provenance: equation, claims, thresholds, and failure envelope frozen before held-out trajectories.
...
- Failure condition: abandon/merge if consequences disappear under payload replacement, atomic firing manufactures causality, or held-out non-resource phenomena require outcomes encoded only as transition payload.
- Provenance: exact structure, worked marking, payload swap, and failure conditions frozen before evaluator review.

## L1

- View: the world relevant to an agent is a family of interaction traces among roles, probes, permitted moves, and response constraints.
- Assumptions: only probe-accessible distinctions count; role/probe boundaries and continuation rules are explicit.
- Structure: trace prefixes and allowed response sets are described; two histories are equivalent when all declared future probes receive the same allowed response sets.
- Native claim: the dossier lists three continuations for a sample prefix and says adding probe `p*` separates two previously equivalent histories.
- Evidence limitation: the continuation list is hand-enumerated, no algorithm/rules sufficient to regenerate it are given, and equivalence checking is not replayed.
- Failure condition: if non-interactive facts or hidden constitution repeatedly control future behavior without appearing in probes, merge with a richer model or abandon interface sufficiency.
- Provenance: prose dossier/failure condition frozen before review.
...
- Structure: static labeled graph; query returns the label on the matching node/edge.
