...
class WorkerRuntime:
    ...
    def __init__(self, *, worker: WorkerIdentity, backend: ExecutionBackend, repo_root: Path, output_dir: Path, lease_ttl_seconds: int = 300, git_head_resolver: Callable[[Path], str] = _resolve_git_head, command_runner: Callable[..., subprocess.CompletedProcess[str]] = subprocess.run):
        worker.validate()
    ...
    def run_once(self) -> dict[str, object]:
        ...
        self.backend.start(claim, self.lease_ttl_seconds)
        ...
        for index, command in enumerate(profile.commands, start=1):
            self.backend.heartbeat(claim, self.lease_ttl_seconds)
            try:
        ...
        self.backend.complete(claim, result_id=result_id, verdict=verdict, artifact_locator=str(result_path), artifact_sha256=digest, artifact_byte_size=len(payload_bytes), metadata={"canonical_output": False, "execution_profile_id": profile.execution_profile_id, "execution_profile_sha256": profile.profile_sha256})
        return {"status": "COMPLETED", "worker_id": self.worker.worker_id, "run_id": claim.task.run_id, "result_id": result_id, "verdict": verdict, "artifact_locator": str(result_path), "artifact_sha256": digest, "artifact_byte_size": len(payload_bytes)}
