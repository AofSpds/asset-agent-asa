#!/usr/bin/env python3
"""Least-privilege LIVE adapter for the governed Finance Page100 G11C5 route.
...
the fresh execution claim and G11C5 checkpoint or call the Finance provider.

The public factory is ``create_sealed_g11c5_custody_adapter``.  Tests may inject
...
OWNER_CAP_SPEC_SHA256 = "f912a117a170096d752fe269913848a93614544857c854dad23a2283f5387156"
EXECUTION_TOKEN_SHA256 = "9bb9c44d64b68699d90b9764b1add085515ec43a1958a0fedcc1a62770f465de"
ACTIVATION_BASE_HEAD_COMMIT = "6e4660cfbb1730dcaeaa2908c9e1a38de012a920"
ACTIVATION_BASE_TREE = "3e4a53a6df8ac7fa4f500c51a951ae9c900476d8"
...
GENERATION_TIMESTAMP = "20260901152200"
GENERATION_ID = f"FINANCE-PAGE100-G11C5-{GENERATION_TIMESTAMP}"
RUNTIME_LOCK_ID = f"PMO-FINANCE-PAGE100-G11C5-{GENERATION_TIMESTAMP}"
PILOT_RUN_ID = f"FINANCE-PAGE100-PILOT-G11C5-{GENERATION_TIMESTAMP}"
PREPARATION_ID = f"FINANCE-PAGE100-G11C5-PREPARATION-{GENERATION_TIMESTAMP}"
PRECHECK_ACT_ID = f"FINANCE-PAGE100-PRECHECK-ACT-G11C5-{GENERATION_TIMESTAMP}"
LIVE_ACT_ID = f"FINANCE-PAGE100-LIVE-ACT-G11C5-{GENERATION_TIMESTAMP}"
LATCH_EVENT_ID = f"FINANCE-PAGE100-LATCH-G11C5-{GENERATION_TIMESTAMP}"
...
G11C5_ACQUISITION_CEILING = 1696
EFFECTIVE_NETWORK_ATTEMPT_CEILING = 2000
INHERITED_G10_ATTEMPTS = 4
G11C5_NETWORK_ATTEMPT_CEILING = 1996
...
G11C5_CONTROL_PREFIX = (
    SOURCE_PREFIX + "_pilot_control/"
    f"runtime_lock_id={RUNTIME_LOCK_ID}/pilot_run_id={PILOT_RUN_ID}/"
)
G11C5_CHECKPOINT_KEY = G11C5_CONTROL_PREFIX + "checkpoint.json"
G11C5_TERMINAL_RECEIPT_KEY = G11C5_CONTROL_PREFIX + "terminal-receipt.json"
...
CHECKPOINT_ARTIFACT = "M3TOP3_FINANCE_CA_PAGE100_G11C5_CHECKPOINT_v1.0"
TERMINAL_ARTIFACT = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_LIVE_TERMINAL_v1.0"
)
LIVE_ADAPTER_PATH = "tools/m3top3/finance_page100_g11c5_live_adapter.py"
FACTORY_SYMBOL = "create_sealed_g11c5_custody_adapter"
ADAPTER_INTERFACE_VERSION = "M3TOP3_FINANCE_CA_PAGE100_G11C5_LIVE_ADAPTER_v1.0"
EX_CONFIG = 78
...
def _validate_active_c5_prefixes(raw_prefix: Any, control_prefix: Any) -> None:
    """Reject any historical namespace before accepting exact G11C5 prefixes."""

    for observed, expected in (
        (raw_prefix, G11C5_RAW_PREFIX),
        (control_prefix, G11C5_CONTROL_PREFIX),
    ):
...
def _validate_plan_seed_material_binding(
    ...
    _exact(
        ...
        "control/m3top3/public-data-source-admission/v1.0/"
        "M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0.json",
...
class LiveContract:
    ...
    g11_acquisition_ceiling: int = G11C5_ACQUISITION_CEILING
    g11_attempt_ceiling: int = G11C5_NETWORK_ATTEMPT_CEILING
...
def validate_governance_bundle(
    *, authority_path: Path, plan_path: Path, seed_path: Path,
    ...
    _exact(authority.get("artifact"),
           "M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0",
    ...
    _exact(plan.get("artifact"),
           "M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_PLAN_v1.0",
    ...
    _exact(seed.get("artifact"),
           "M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0",
    ...
    artifact_match = re.fullmatch(
        r"M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_"
    ...
    _exact(precheck.get("terminal_state"),
           "TERMINAL_PASS_FOCUSED_G11C5_PRECHECK_EXACT_3_OIDC_STS_POLICY_"
    ...
    _exact(activation.get("artifact"),
           "M3TOP3_FINANCE_CA_PAGE100_G11C5_ELIGIBLE_SUCCESSOR_LIVE_ACTIVATION_v1.0",
    ...
    actual_head = live_head_sha or os.environ.get("G11C5_LIVE_HEAD_SHA", "")
    actual_tree = live_head_tree or os.environ.get("G11C5_LIVE_HEAD_TREE", "")
    ...
    _exact(os.environ.get("G11C5_LIVE_HEAD_SHA"), actual_head,
           "LIVE_RUNTIME_HEAD_ENV_MISMATCH")
    _exact(os.environ.get("G11C5_LIVE_HEAD_TREE"), actual_tree,
...
def raw_prefix(bas_dt: str, page_no: int, attempt: int) -> str:
    ...
    return (
        G11C5_RAW_PREFIX + f"{OPERATION}/quota_day_kst={QUOTA_DAY_KST}/"
...
LIVE_AWS_CREDENTIAL_ENV = {
    CHECKPOINT_READ_ROLE: (
        "G11C5_CHECKPOINT_READ_AWS_ACCESS_KEY_ID",
        "G11C5_CHECKPOINT_READ_AWS_SECRET_ACCESS_KEY",
        "G11C5_CHECKPOINT_READ_AWS_SESSION_TOKEN",
    ),
    RAW_READ_ROLE: (
        "G11C5_RAW_FOUR_READ_AWS_ACCESS_KEY_ID",
        "G11C5_RAW_FOUR_READ_AWS_SECRET_ACCESS_KEY",
        "G11C5_RAW_FOUR_READ_AWS_SESSION_TOKEN",
    ),
    FINAL_LIST_WRITE_ROLE: (
        "G11C5_FINAL_LIST_WRITE_AWS_ACCESS_KEY_ID",
        "G11C5_FINAL_LIST_WRITE_AWS_SECRET_ACCESS_KEY",
        "G11C5_FINAL_LIST_WRITE_AWS_SESSION_TOKEN",
    ),
}
...
def load_live_credential_bundles(
    ...
    if missing or len(bundles) != 3:
        raise GovernanceError("G11C5_EXACT_THREE_CREDENTIAL_BUNDLES_REQUIRED")
    fingerprints = {
    ...
    if len(fingerprints) != 3:
        raise GovernanceError("G11C5_CREDENTIAL_BUNDLES_MUST_BE_DISTINCT")
...
class AwsCliS3ObjectStore:
    ...
    def pre_mutation_gate(self) -> None:
        ...
        if (
            ...
            or not G11C5_RAW_PREFIX.startswith(SOURCE_PREFIX)
            or not G11C5_CONTROL_PREFIX.startswith(SOURCE_PREFIX)
            or not EXECUTION_CLAIM_KEY.startswith(SOURCE_PREFIX)
        ):
            raise GovernanceError("STATIC_CUSTODY_SCOPE_MISMATCH")
        for prefix in (G11C5_RAW_PREFIX, G11C5_CONTROL_PREFIX, EXECUTION_CLAIM_KEY):
            query = (
            ...
            if (
                ...
                raise GovernanceError("FRESH_G11C5_IDENTITY_ALREADY_CONSUMED")
    ...
    def create_once(
        ...
        if not (
            ...
            or key.startswith(G11C5_RAW_PREFIX)
            or key.startswith(G11C5_CONTROL_PREFIX)
        ):
    ...
    def compare_and_swap(
        ...
        if self.credential_role != FINAL_LIST_WRITE_ROLE or not self._final_gate_complete:
            raise GovernanceError("FINAL_WRITE_PHASE_NOT_OPEN")
        if key != G11C5_CHECKPOINT_KEY or not expected_etag:
...
class PhaseSeparatedS3ObjectStore:
    ...
    def __init__(
        ...
        if (
            ...
            raise GovernanceError("G11C5_PHASE_CLIENT_BINDING_INVALID")
    ...
    def exact_read(self, binding: ObjectBinding) -> VersionedObject:
        ...
        if self._final_open or self._read_index >= len(expected) or binding != expected[self._read_index]:
            raise GovernanceError("G11C5_ONE_WAY_READ_PHASE_VIOLATION")
    ...
    def pre_mutation_gate(self) -> None:
        if self._read_index != 5 or self._final_open:
            raise GovernanceError("G11C5_FINAL_PHASE_HANDOFF_INVALID")
    ...
    def create_once(
        ...
        if not self._final_open:
            raise GovernanceError("G11C5_FINAL_PHASE_NOT_OPEN")
    ...
    def fetch_once(self, params: Mapping[str, str]) -> ProviderResponse:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "AAA-M3Top3-Finance-Page100-G11C5/1.0",
            },
        )
...
class G11LiveAdapter:
    ...
    def _save_checkpoint(self, *, initial: bool = False) -> None:
        ...
        if self.effects.checkpoint_writes >= CHECKPOINT_WRITE_CEILING:
            raise BudgetError("G11C5_CHECKPOINT_WRITE_CEILING")
        if not initial:
        ...
        if initial:
            observed = self.store.create_once(
                G11C5_CHECKPOINT_KEY, body, content_type="application/json", metadata=metadata
            )
        else:
            if self.checkpoint_object is None:
                raise CustodyError("CHECKPOINT_CAS_TOKEN_MISSING")
            observed = self.store.compare_and_swap(
                G11C5_CHECKPOINT_KEY, body, expected_etag=self.checkpoint_object.etag,
    ...
    def _assert_checkpoint(self) -> None:
        ...
        if (
            ...
            raise CustodyError("G11C5_CHECKPOINT_BINDING_MISMATCH")
        ...
        if not all(isinstance(v, list) for v in (attempts, raw_index, slots)):
            raise CustodyError("G11C5_CHECKPOINT_COLLECTION_INVALID")
        if not isinstance(budget, Mapping) or not isinstance(projection, Mapping):
            raise CustodyError("G11C5_CHECKPOINT_MAPPING_INVALID")
        if (
            ...
            raise CustodyError("G11C5_CHECKPOINT_COUNTER_MISMATCH")
        if (
            ...
            raise CustodyError("G11C5_PROJECTION_ACCOUNTING_MISMATCH")

    def _claim(self) -> None:
        if self.effects.execution_claim_writes >= EXECUTION_CLAIM_WRITE_CEILING:
            raise BudgetError("G11C5_EXECUTION_CLAIM_WRITE_CEILING")
        ...
        claim = {
            "artifact": "M3TOP3_FINANCE_PAGE100_G11C5_EXECUTION_CLAIM_v1.0",
    ...
    def _reserve(self, bas_dt: str, page_no: int) -> dict[str, Any]:
        ...
        if len(self.checkpoint["attempts"]) >= self.contract.g11_attempt_ceiling:
            raise BudgetError("G11C5_NETWORK_ATTEMPT_CEILING")
        if slot not in self.checkpoint["unique_page_slots"]:
            if len(self.checkpoint["unique_page_slots"]) >= self.contract.g11_acquisition_ceiling:
                raise BudgetError("G11C5_ACQUISITION_CEILING")
    ...
    def _call_and_custody(
        ...
        if self.effects.raw_writes >= self.contract.g11_acquisition_ceiling:
            raise BudgetError("G11C5_RAW_WRITE_CEILING")
    ...
    def _result(self, verdict: str, error: LiveAdapterError | None = None) -> dict[str, Any]:
        ...
        result = {
            ...
            "governed_correction_commit": self.contract.correction_head,
            "terminal_state": (
                "TERMINAL_PASS_BOUNDED_G11C5_DATA_GENERATION_SOURCE_NOT_ADMITTED"
            ...
            "terminal_receipt_binding": {
                "key": G11C5_TERMINAL_RECEIPT_KEY,
    ...
    def _terminalize(self, result: Mapping[str, Any]) -> VersionedObject | None:
        ...
        if self.effects.terminal_receipt_put_attempts >= TERMINAL_RECEIPT_WRITE_CEILING:
            raise BudgetError("G11C5_TERMINAL_RECEIPT_WRITE_CEILING")
        receipt = {
        ...
        self.effects.s3_put_calls += 1
        observed = self.store.create_once(
            G11C5_TERMINAL_RECEIPT_KEY, body,
            content_type="application/json", metadata=metadata,
        )
        if observed.body != body:
...
def create_sealed_g11c5_custody_adapter(
    *, authority_path: Path, plan_path: Path, seed_path: Path,
    manifest_path: Path, owner_decision_path: Path,
    live_activation_path: Path, precheck_receipt_path: Path,
...
class _LiveAdapterEntrypoint:
    ...
    def execute(self, request: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if not isinstance(request, Mapping) or any(
            ...
            return result
        try:
            adapter = create_sealed_g11c5_custody_adapter(
