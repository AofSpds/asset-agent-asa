#!/usr/bin/env python3
"""Fresh G10 read-only forensic S2 control wrapper.
...
GENERATION_STAMP = "20260830071905"
GENERATION_ID = "G10-READONLY-FORENSIC-S2-" + GENERATION_STAMP
RUNTIME_LOCK_ID = "PMO-G10-READONLY-FORENSIC-S2-" + GENERATION_STAMP
PRECHECK_ACT_ID = "G10-READONLY-FORENSIC-S2-PRECHECK-" + GENERATION_STAMP
AUDIT_ACT_ID = "G10-READONLY-FORENSIC-S2-AUDIT-" + GENERATION_STAMP
ACTIVATION_ID = "G10-READONLY-FORENSIC-S2-ACTIVATION-" + GENERATION_STAMP

CURRENT_TERMINAL_HEAD = "a9425e3edc1661f78071aafa10a559abf7804f1e"
CURRENT_TERMINAL_TREE = "5b7a17ea3a3ab4b63177c55f4ec1c0e356a08106"
FAILED_RUN_ID = 33289340151
FAILED_JOB_ID = 99198097639
FAILED_RUN_ATTEMPT = 1
FAILED_ACTIVATION_HEAD = "0efc4beccd3e3b4dc82b56cf6e6f4efe3ac291a0"
FAILED_EXACT_CODE = "EVENT_ENV_EVENT_FORCED_MISMATCH"
FAILED_RECEIPT_PATH = (
    "control/m3top3/public-data-source-admission/v1.0/"
    "M3TOP3_FINANCE_CA_PAGE100_G10_READ_ONLY_FORENSIC_TERMINAL_RECEIPT_"
    "33289340151_v1.0.json"
)
FAILED_RECEIPT_BYTES = 4523
FAILED_RECEIPT_BLOB = "81833f6f10b351a9b0f18667dfa6540a7bba9b40"
FAILED_RECEIPT_SHA256 = (
    "eec1f876b97c2347dce35394031dc6f868c25b38670e3930152e8101701282a9"
)
...
PREP_MESSAGE = "Prepare G10 read-only forensic S2 20260830071905 v1.0"
PRECHECK_MESSAGE = "Arm G10 read-only forensic S2 PRECHECK 20260830071905 v1.0"
AUDIT_MESSAGE = "Arm G10 read-only forensic S2 AUDIT 20260830071905 v1.0"

ROOT = pathlib.Path(__file__).resolve().parents[2]
BASE_DIR = "control/m3top3/public-data-source-admission/v1.0"
PRECHECK_WORKFLOW_PATH = (
    ".github/workflows/"
    "m3top3-finance-page100-g10-readonly-forensic-s2-precheck-v1.yml"
)
AUDIT_WORKFLOW_PATH = (
    ".github/workflows/"
    "m3top3-finance-page100-g10-readonly-forensic-s2-audit-v1.yml"
)
RUNNER_PATH = "tools/m3top3/finance_page100_g10_forensic_s2.py"
TEST_PATH = "tools/m3top3/tests/test_finance_page100_g10_forensic_s2.py"
AUTHORITY_PATH = (
    BASE_DIR + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AUTHORITY_v1.0.json"
)
MANIFEST_PATH = (
    BASE_DIR + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_MANIFEST_v1.0.json"
)
PRECHECK_POLICY_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_SESSION_POLICY_v1.0.json"
)
AUDIT_POLICY_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AUDIT_SESSION_POLICY_v1.0.json"
)
PRECHECK_TEMPLATE_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_ACTIVATION_v1.0.json.template"
)
AUDIT_TEMPLATE_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AUDIT_ACTIVATION_v1.0.json.template"
)
PRECHECK_ACTIVATION_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_ACTIVATION_v1.0.json"
)
AUDIT_ACTIVATION_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AUDIT_ACTIVATION_v1.0.json"
)
PRECHECK_PASS_RECEIPT_PATH = (
    BASE_DIR
    + "/M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_PASS_RECEIPT_v1.0.json"
)
...
class S2ForensicError(RuntimeError):
    """Fail-closed S2 control error."""


def require(condition: bool, code: str) -> None:
    if not condition:
        raise S2ForensicError(code)
...
def load_canonical_json(path: pathlib.Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise S2ForensicError("JSON_PARSE_INVALID") from exc
...
def parse_canonical_json_bytes(raw: bytes, code: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise S2ForensicError(code + "_PARSE") from exc
...
def validate_policy(policy: Mapping[str, Any], mode: str) -> None:
    ...
    expected_statement_count = 1 if mode == "PRECHECK" else 4
    require(
    ...
    require(
        statements[1].get("Resource") == bucket + "/" + frozen.RAW_PREFIX + "*"
        and statements[1]
        .get("Condition", {})
        .get("StringEquals", {})
        .get("s3:VersionId")
        == [row["version_id"] for row in EXACT_RAW_VERSIONS],
        "POLICY_RAW_GET_SCOPE_INVALID",
    )
    require(
        statements[2].get("Resource") == bucket + "/" + frozen.CHECKPOINT_KEY
        and "Condition" not in statements[2],
        "POLICY_CHECKPOINT_GET_SCOPE_INVALID",
    )
    require(
        statements[3].get("Resource") == bucket + "/" + frozen.CLAIM_KEY
        and statements[3]
        .get("Condition", {})
        .get("StringEquals", {})
        .get("s3:VersionId")
        == "27scTNMVYtc.VuIP2xEo.qqPKN3WQiPf",
        "POLICY_CLAIM_GET_SCOPE_INVALID",
    )
    require(
        len(canonical_json_bytes(dict(policy))) - 1 <= 2048,
        "POLICY_STS_INLINE_SIZE_EXCEEDED",
    )
...
def validate_s2_contract(
    ...
    require(
        ...
        == "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AUTHORITY_v1.0",
        "S2_AUTHORITY_ARTIFACT_INVALID",
    )
    require(
        authority.get("state")
        == "OWNER_AUTHORIZED_LOGICAL_READ_ONLY_FORENSIC_SUCCESSOR",
        "S2_AUTHORITY_STATE_INVALID",
    )
    require(
        authority.get("repository") == REPOSITORY
        and authority.get("branch") == BRANCH,
        "S2_AUTHORITY_REPO_BRANCH_INVALID",
    )
    require(authority.get("owner_authorization") == _owner_binding(), "S2_OWNER_BINDING_INVALID")
    ...
    require(
        ...
        "S2_SOURCE_SCOPE_BINDING_INVALID",
    )
    ...
    require(
        ...
        "S2_PREDECESSOR_TERMINAL_INVALID",
    )
    require(authority.get("authorized_effects") == ZERO_EFFECTS, "S2_EFFECTS_NOT_ZERO")
    require(authority.get("g10_live_rerun_authorized") is False, "S2_G10_RERUN_ENABLED")
    require(authority.get("g11_authorized") is False, "S2_G11_ENABLED")
    require(authority.get("semantic_change_authorized") is False, "S2_SEMANTIC_CHANGE_ENABLED")
    require(authority.get("normalization_promotion_release_production_authorized") is False, "S2_RELEASE_SURFACE_ENABLED")
    aws = authority.get("aws_read_only_scope", {})
    require(
        aws.get("account_id") == ACCOUNT_ID
        and aws.get("region") == REGION
        and aws.get("bucket") == BUCKET,
        "S2_AWS_SCOPE_INVALID",
    )
    require(
    ...
    require(
        ...
        "S2_FAILED_RECEIPT_BYTES_INVALID",
    )
    ...
    require(
        ...
        "S2_FAILED_RECEIPT_CONTENT_INVALID",
    )
    ...
    require(
        ...
        "S2_POLICY_BYTES_INVALID",
    )
    bindings = authority.get("execution_bindings")
    require(isinstance(bindings, dict), "S2_EXECUTION_BINDINGS_INVALID")
    for key, relative in (
        ...
        require(
            ...
            "S2_EXECUTION_BINDING_INVALID_" + key,
        )

    require(
        manifest.get("artifact")
        == "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_MANIFEST_v1.0",
        "S2_MANIFEST_ARTIFACT_INVALID",
    )
    require(
    ...
    require(
        ...
        "S2_MANIFEST_FILE_SET_INVALID",
    )
    for relative, binding in manifest_files.items():
        require(binding == file_binding(relative), "S2_MANIFEST_BINDING_INVALID_" + relative)

    require(
        activation.get("artifact")
        == (
            "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_ACTIVATION_v1.0"
            if mode == "PRECHECK"
            else "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AUDIT_ACTIVATION_v1.0"
        ),
        "S2_ACTIVATION_ARTIFACT_INVALID",
    )
    require(activation.get("mode") == mode, "S2_ACTIVATION_MODE_INVALID")
    require(
        activation.get("state")
        == ("ARMED_FRESH_PRECHECK_ONCE" if mode == "PRECHECK" else "ARMED_FRESH_AUDIT_ONCE"),
        "S2_ACTIVATION_STATE_INVALID",
    )
    require(
        activation.get("repository") == REPOSITORY
        and activation.get("branch") == BRANCH
        and activation.get("owner_actor") == ACTOR,
        "S2_ACTIVATION_REPO_BRANCH_INVALID",
    )
    require(
        activation.get("activation_commit_message") == _message_for(mode),
        "S2_ACTIVATION_MESSAGE_INVALID",
    )
    require(activation.get("owner_authorization") == _owner_binding(), "S2_ACTIVATION_OWNER_INVALID")
    require(activation.get("successor_identity") == successor, "S2_ACTIVATION_IDENTITY_INVALID")
    require(
        ...
        "S2_ACTIVATION_TERMINAL_BASE_INVALID",
    )
    require(activation.get("authorized_effects") == ZERO_EFFECTS, "S2_ACTIVATION_EFFECTS_INVALID")
    require(activation.get("g10_live_rerun") is False and activation.get("g11_generation") is False, "S2_ACTIVATION_SCOPE_INVALID")
    require(
        ...
        "S2_ACTIVATION_CONSUMED_PREDECESSOR_INVALID",
    )
    require(
        activation.get("execution_bindings") == bindings,
        "S2_ACTIVATION_EXECUTION_BINDINGS_INVALID",
    )
    for key in ("preparation_head_sha", "preparation_tree_sha"):
        require(
            bool(re.fullmatch(r"[0-9a-f]{40}", str(activation.get(key, "")))),
            "S2_ACTIVATION_PREPARATION_ID_INVALID",
        )
    if mode == "PRECHECK":
        require("precheck_evidence" not in activation, "S2_PRECHECK_SELF_EVIDENCE_PRESENT")
    else:
        proof = activation.get("precheck_evidence")
        require(isinstance(proof, dict), "S2_AUDIT_PRECHECK_EVIDENCE_MISSING")
        require(
            ...
            "S2_AUDIT_PRECHECK_GIT_ID_INVALID",
        )
        require(
            ...
            "S2_AUDIT_PRECHECK_RUNTIME_ID_INVALID",
        )
        require(
            ...
            "S2_AUDIT_PRECHECK_RESULT_INVALID",
        )
        require(
            proof.get("artifact_name")
            == f"m3top3-g10-readonly-forensic-s2-precheck-{proof['run_id']}-1",
            "S2_AUDIT_PRECHECK_ARTIFACT_NAME_INVALID",
        )
        require(
            type(proof.get("artifact_size_in_bytes")) is int
            and proof.get("artifact_size_in_bytes") > 0,
            "S2_AUDIT_PRECHECK_ARTIFACT_SIZE_INVALID",
        )
        pass_binding = proof.get("pass_receipt")
        require(isinstance(pass_binding, dict), "S2_PRECHECK_PASS_BINDING_MISSING")
        require(
            pass_binding.get("path") == PRECHECK_PASS_RECEIPT_PATH
            and pass_binding.get("commit_message")
            == f"Seal G10 read-only forensic S2 PRECHECK PASS {proof['run_id']} v1.0"
            and bool(
                re.fullmatch(
            ...
            "S2_PRECHECK_PASS_GIT_BINDING_INVALID",
        )
        ...
        require(
            ...
            "S2_PRECHECK_PASS_FILE_BINDING_INVALID",
        )
        pass_receipt = load_canonical_json(ROOT / PRECHECK_PASS_RECEIPT_PATH)
        require(
            pass_receipt.get("artifact")
            == "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_PASS_RECEIPT_v1.0"
            ...
            "S2_PRECHECK_PASS_CONTENT_INVALID",
        )
    ...
    require(
        ...
        "S2_EXACT_RAW_VERSION_BINDINGS_INVALID",
    )
    ...
    require(
        ...
        "S2_EXACT_HISTORY_EXPECTATIONS_INVALID",
    )
    ...
    require(
        ...
        "S2_FROZEN_ALGORITHM_BINDING_INVALID",
    )
...
def verify_remote_gate(
    ...
    require(active_ids == {current_run_id}, "DUPLICATE_OR_FOREIGN_ACTIVE_S2_RUN")
    preparation_runs = _workflow_runs(
        token, str(activation["preparation_head_sha"]), mode
    )
    require(preparation_runs == [], "PREPARATION_PUSH_TRIGGERED_S2_WORKFLOW")
    result = {
        "phase": phase,
        "mode": mode,
...
def _aws_json(service: str, operation: str, *args: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise S2ForensicError("PRECHECK_AWS_JSON_INVALID") from exc
...
def _finalize_output(
    ...
    scan = {
        "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_EXACT_SECRET_SCAN_v1.0",
        "state": "PASS",
    ...
    sanitization = {
        "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_SANITIZATION_RECEIPT_v1.0",
        "state": "PASS",
        "files": files,
...
def run_precheck_aws(
    ...
    expected_session_arn = (
        ...
        f"g10-readonly-forensic-s2-precheck-{os.environ.get('GITHUB_RUN_ID', '')}"
    )
    ...
    activation_readback = {
        "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_ACTIVATION_READBACK_v1.0",
        "state": "PASS",
        "mode": "PRECHECK",
        "generation_id": GENERATION_ID,
    ...
    precheck_receipt = {
        "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_RECEIPT_v1.0",
    ...
    aws_receipt = {
        "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_AWS_READONLY_SESSION_RECEIPT_v1.0",
        "state": "PASS",
        "account_id": ACCOUNT_ID,
        "region": REGION,
        "bucket": BUCKET,
    ...
    terminal = {
        "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_TERMINAL_SUMMARY_v1.0",
...
class BoundedFrozenAwsReadOnlyS3(frozen.AwsReadOnlyS3):
    ...
    def aws_json(self, *args: str) -> dict[str, Any]:
        ...
        if name == "sts:get-caller-identity":
            expected_arn = (
                f"arn:aws:sts::{ACCOUNT_ID}:assumed-role/M3Top3GitHubOIDCRawWriter/"
                f"g10-readonly-forensic-s2-audit-{os.environ.get('GITHUB_RUN_ID', '')}"
            )
            require(
    ...
    def list_versions(self, prefix: str) -> dict[str, list[dict[str, Any]]]:
        ...
        inventory = frozen.paginate_version_pages(
            lambda key_marker, version_marker: (
                ...
                else (_ for _ in ()).throw(S2ForensicError("AUDIT_SECOND_LIST_PAGE_PROHIBITED"))
            ),
            prefix,
        )
        self._listed_pairs.update(
    ...
    def read_exact_version(
        ...
        *,
        ...
        require(
            (key, version_id) in self._listed_pairs,
            "AUDIT_GET_PAIR_NOT_IN_EXACT_INVENTORY",
        )
...
def verify_precheck_archive(
    ...
    try:
        ...
        raise S2ForensicError("PRECHECK_ARCHIVE_ZIP_INVALID") from exc
    ...
    expected_role_arn = (
        f"arn:aws:sts::{ACCOUNT_ID}:assumed-role/M3Top3GitHubOIDCRawWriter/"
        f"g10-readonly-forensic-s2-precheck-{proof['run_id']}"
    )
    expected_role_hash = sha256_bytes(expected_role_arn.encode("utf-8"))
    require(
        receipt.get("artifact")
        == "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_PRECHECK_RECEIPT_v1.0"
...
def run_audit_projection(
    ...
    with tempfile.TemporaryDirectory(prefix="g10-s2-frozen-core-") as temp_name:
    ...
    write_canonical_json(
        ...
        {
            "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_RAW_VERSION_MANIFEST_v1.0",
    ...
    write_canonical_json(
        ...
        {
            "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_CHECKPOINT_VERSION_HISTORY_v1.0",
    ...
    write_canonical_json(
        ...
        {
            "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_EXECUTION_CLAIM_VERSION_HISTORY_v1.0",
    ...
    write_canonical_json(
        ...
        {
            "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_ISSUER_CONFLICT_REPRODUCTION_v1.0",
    ...
    write_canonical_json(
        ...
        {
            "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_FORENSIC_S2_TERMINAL_SUMMARY_v1.0",
...
def _load_s2(
    ...
    validate_s2_contract(authority, activation, manifest, policy, mode)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    authority, activation, _, _ = _load_s2(
        args.authority,
        args.activation,
        args.manifest,
        args.policy,
    ...
    if args.command == "precheck-aws":
        ...
        print("G10_READ_ONLY_FORENSIC_S2_PRECHECK_COMPLETE")
        return 0
    ...
    print("G10_READ_ONLY_FORENSIC_S2_AUDIT_COMPLETE")
    return 0
