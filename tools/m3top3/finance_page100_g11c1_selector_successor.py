#!/usr/bin/env python3
"""Fail-closed core and focused PRECHECK for the Finance Page100 G11C1 successor.
...
GENERATION_TIMESTAMP = "20260901123521"
GENERATION_ID = f"FINANCE-PAGE100-G11C1-{GENERATION_TIMESTAMP}"
RUNTIME_LOCK_ID = f"PMO-FINANCE-PAGE100-G11C1-{GENERATION_TIMESTAMP}"
PILOT_RUN_ID = f"FINANCE-PAGE100-PILOT-G11C1-{GENERATION_TIMESTAMP}"
PRECHECK_ACT_ID = f"FINANCE-PAGE100-PRECHECK-ACT-G11C1-{GENERATION_TIMESTAMP}"
LIVE_ACT_ID = f"FINANCE-PAGE100-LIVE-ACT-G11C1-{GENERATION_TIMESTAMP}"
LATCH_EVENT_ID = f"FINANCE-PAGE100-LATCH-G11C1-{GENERATION_TIMESTAMP}"
...
ACTIVATION_BASE_HEAD_COMMIT = "e8b0b93714060627b2fbc124566eb6a5b32cf9d5"
ACTIVATION_BASE_TREE = "0d4465091a680c1ac9ad6c7aed3aed8f606f57ea"
...
OWNER_CAP_SPEC_SHA256 = "5eae2419731d045b6dbaa8795a42c430d0efc42b54f897a1618b09c4573ccde2"
EXECUTION_TOKEN_SHA256 = "a9bd3a1bfacd0a04e9ab76b80aa4ec3f795258251fdc30b37409ca5a8c56fec6"
...
AUTHORITY_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0.json"
)
PLAN_FILENAME = "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_PLAN_v1.0.json"
SEED_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0.json"
)
MANIFEST_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_MANIFEST_v1.0.json"
)

AUTHORITY_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0"
PLAN_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_PLAN_v1.0"
SEED_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0"
MANIFEST_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_MANIFEST_v1.0"

LIVE_ADAPTER_GATE_BLOCKED = "BLOCKED_MISSING_EXECUTABLE_CUSTODY_ADAPTERS"
LIVE_ADAPTER_GATE_READY = "READY"
LIVE_ADAPTER_REPO_PATH = "tools/m3top3/finance_page100_g11c1_live_adapter.py"
LIVE_ADAPTER_INTERFACE_VERSION = "M3TOP3_FINANCE_CA_PAGE100_G11C1_LIVE_ADAPTER_v1.0"
LIVE_ADAPTER_FACTORY_SYMBOL = "create_sealed_g11c1_custody_adapter"
LIVE_ADAPTER_TEST_REPO_PATH = "tools/m3top3/tests/test_finance_page100_g11c1_live_adapter.py"
...
def load_sealed_live_adapter_factory(adapter_path: Path, expected_sha256: str) -> Any:
    ...
    module_name = f"_m3top3_g11c1_live_adapter_{expected_sha256}"
    try:
...
def validate_g11_object_key(key: str) -> None:
    """Reject writes outside the fresh G11C1 namespace or historical namespaces."""

    _require(isinstance(key, str) and bool(key), "INVALID_G11_OBJECT_KEY", "object key is empty")
    _require("G11C1" in key and GENERATION_TIMESTAMP in key,
             "INVALID_G11_OBJECT_KEY", "key is not bound to the fresh G11C1 generation")
...
def expected_owner_cap_spec() -> dict[str, Any]:
    return {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C1_OWNER_CAP_SPEC_v1.0",
        ...
        "g10_spent_primary_acquisitions": INHERITED_G10_ACQUISITIONS,
        "g10_spent_network_attempts": INHERITED_G10_ATTEMPTS,
        "maximum_new_g11c1_primary_acquisitions": G11_ACQUISITION_CEILING,
        "maximum_new_g11c1_network_attempts": G11_ATTEMPT_CEILING,
...
def expected_execution_token_material() -> dict[str, Any]:
    return {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C1_EXECUTION_TOKEN_MATERIAL_v1.0",
        "generation_id": GENERATION_ID,
        "runtime_lock_id": RUNTIME_LOCK_ID,
        "pilot_run_id": PILOT_RUN_ID,
        "precheck_act_id": PRECHECK_ACT_ID,
...
def _validate_zero_precheck_receipt(receipt: Mapping[str, Any]) -> str:
    ...
    match = re.fullmatch(
        r"M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_"
    ...
    _require_exact(
        ...
        "TERMINAL_PASS_FOCUSED_G11C1_PRECHECK_ZERO_EXTERNAL_EFFECT_LIVE_NOT_AUTHORIZED",
...
def validate_runtime_live_head_binding(
    ...
    *,
    ...
    head_sha = env.get("G11C1_LIVE_HEAD_SHA", "")
    tree_sha = env.get("G11C1_LIVE_HEAD_TREE", "")
    _require(isinstance(head_sha, str) and HEX40.fullmatch(head_sha) is not None,
             "LIVE_RUNTIME_HEAD_INVALID", "G11C1_LIVE_HEAD_SHA")
    _require(isinstance(tree_sha, str) and HEX40.fullmatch(tree_sha) is not None,
             "LIVE_RUNTIME_TREE_INVALID", "G11C1_LIVE_HEAD_TREE")
    _require_exact(env.get("GITHUB_SHA"), head_sha, "GITHUB_SHA/G11C1_LIVE_HEAD_SHA")
...
def validate_live_activation_and_receipt(
    *,
    ...
    _require_exact(activation.get("artifact"),
                   "M3TOP3_FINANCE_CA_PAGE100_G11C1_ELIGIBLE_SUCCESSOR_LIVE_ACTIVATION_v1.0",
...
def _normalize_and_validate_live_result(
    ...
    *,
    ...
    _require_exact(output.get("schema"),
                   "M3TOP3_FINANCE_CA_PAGE100_G11C1_LIVE_ENTRY_RESULT_v1.0",
...
def _precheck(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    return 0, {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C1_FOCUSED_PRECHECK_RESULT_v1.0",
        "verdict": "PASS",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.mode == "live":
            ...
            result = {
                "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C1_GATE_FAILURE_v1.0",
                "verdict": "FAIL_CLOSED",
