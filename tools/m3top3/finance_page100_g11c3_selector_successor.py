#!/usr/bin/env python3
"""Fail-closed core and focused PRECHECK for the Finance Page100 G11C3 successor.
...
validation, and a zero-effect PRECHECK CLI.  ``--mode live`` imports and invokes
only the separately reviewed, exact-hash-bound custody adapter after a fresh LIVE
activation and zero-effect PRECHECK PASS receipt have both been verified.
"""
...
GENERATION_TIMESTAMP = "20260901134119"
GENERATION_ID = f"FINANCE-PAGE100-G11C3-{GENERATION_TIMESTAMP}"
RUNTIME_LOCK_ID = f"PMO-FINANCE-PAGE100-G11C3-{GENERATION_TIMESTAMP}"
PILOT_RUN_ID = f"FINANCE-PAGE100-PILOT-G11C3-{GENERATION_TIMESTAMP}"
PRECHECK_ACT_ID = f"FINANCE-PAGE100-PRECHECK-ACT-G11C3-{GENERATION_TIMESTAMP}"
LIVE_ACT_ID = f"FINANCE-PAGE100-LIVE-ACT-G11C3-{GENERATION_TIMESTAMP}"
LATCH_EVENT_ID = f"FINANCE-PAGE100-LATCH-G11C3-{GENERATION_TIMESTAMP}"
...
ACTIVATION_BASE_HEAD_COMMIT = "5f400498c0890d756b3d5cbe6ede7ec6d2292450"
ACTIVATION_BASE_TREE = "b5e5eb8c2d08feaa99e83185ee1ef0eaf8e90004"
...
OWNER_CAP_SPEC_SHA256 = "1c92a1096361a2fb0fd37c172cc14f3a94bcb5e1a84396f53f0185478bdfbf1d"
EXECUTION_TOKEN_SHA256 = "f26509a72b1bad3eabdab55357df1126567c426bdd5a70061f8a85e137f351a6"
...
AUTHORITY_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0.json"
)
PLAN_FILENAME = "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_PLAN_v1.0.json"
SEED_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0.json"
)
MANIFEST_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_MANIFEST_v1.0.json"
)

AUTHORITY_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0"
PLAN_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_PLAN_v1.0"
SEED_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0"
MANIFEST_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_MANIFEST_v1.0"

LIVE_ADAPTER_GATE_BLOCKED = "BLOCKED_MISSING_EXECUTABLE_CUSTODY_ADAPTERS"
LIVE_ADAPTER_GATE_READY = "READY"
LIVE_ADAPTER_REPO_PATH = "tools/m3top3/finance_page100_g11c3_live_adapter.py"
LIVE_ADAPTER_INTERFACE_VERSION = "M3TOP3_FINANCE_CA_PAGE100_G11C3_LIVE_ADAPTER_v1.0"
LIVE_ADAPTER_FACTORY_SYMBOL = "create_sealed_g11c3_custody_adapter"
LIVE_ADAPTER_TEST_REPO_PATH = "tools/m3top3/tests/test_finance_page100_g11c3_live_adapter.py"
...
# This is the exact output surface consumed by the LIVE workflow.  It is used
# only before the sealed adapter is invoked.  Once adapter execution begins the
# adapter's observed ledger is preserved, including an ambiguity flag on any
# unreconciled effect; it is never replaced by a synthetic zero ledger.
LIVE_ZERO_EFFECTS: dict[str, int | bool] = {
...
def validate_consumed_predecessor_identities(
    ...
    """Require both G11C1 and G11C2 identities and the C2 PRECHECK to be consumed."""
...
def validate_live_session_policy_for_aws(path: Path) -> int:
    ...
    _require("Version" not in parsed, "LIVE_SESSION_POLICY_VERSION_MUST_BE_OMITTED",
             "G11C3 inline policy must use the permission-equivalent default version")
    _require("${" not in source_text, "LIVE_SESSION_POLICY_VARIABLE_FORBIDDEN",
             "G11C3 inline policy may not contain policy variables")
    _require_exact(parsed, expected_live_session_policy(), "LIVE session policy semantics")
...
def load_sealed_live_adapter_factory(adapter_path: Path, expected_sha256: str) -> Any:
    ...
    module_name = f"_m3top3_g11c3_live_adapter_{expected_sha256}"
    try:
...
def validate_g11_object_key(key: str) -> None:
    """Reject writes outside the fresh G11C3 namespace or historical namespaces."""

    _require(isinstance(key, str) and bool(key), "INVALID_G11_OBJECT_KEY", "object key is empty")
    _require("G11C3" in key and GENERATION_TIMESTAMP in key,
             "INVALID_G11_OBJECT_KEY", "key is not bound to the fresh G11C3 generation")
    for forbidden in (
...
def validate_active_c3_prefixes(raw_prefix: Any, control_prefix: Any) -> None:
    """Reject adoption of a G11/G11C1/G11C2 namespace as the active C3 route."""
...
def validate_authority_document(document: Mapping[str, Any]) -> None:
    ...
    _require_exact(route.get("one_fresh_zero_effect_precheck_authorized"), True,
                   "authority.authorized_route.one_fresh_zero_effect_precheck_authorized")
    ...
    _require(isinstance(consumed, list) and all(run in consumed for run in REQUIRED_NO_RERUN_RUNS),
             "NO_RERUN_BINDING_MISSING", "all consumed G10/S2/S3 runs must be bound")
...
def expected_adapter_execution_order() -> dict[str, Any]:
    return {
        ...
        "precheck_performs_runtime_aws_reads": False,
...
def expected_owner_cap_spec() -> dict[str, Any]:
    return {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C3_OWNER_CAP_SPEC_v1.0",
        ...
        "g10_spent_primary_acquisitions": INHERITED_G10_ACQUISITIONS,
        "g10_spent_network_attempts": INHERITED_G10_ATTEMPTS,
        "maximum_new_g11c3_primary_acquisitions": G11_ACQUISITION_CEILING,
        "maximum_new_g11c3_network_attempts": G11_ATTEMPT_CEILING,
...
def expected_execution_token_material() -> dict[str, Any]:
    return {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C3_EXECUTION_TOKEN_MATERIAL_v1.0",
        "generation_id": GENERATION_ID,
        "runtime_lock_id": RUNTIME_LOCK_ID,
        "pilot_run_id": PILOT_RUN_ID,
        "precheck_act_id": PRECHECK_ACT_ID,
...
def validate_plan_document(document: Mapping[str, Any]) -> None:
    ...
    _require_exact(
        ...
        "SEALED_STATIC_GOVERNED_EVIDENCE_VALIDATION_ONLY_ZERO_AWS_ZERO_EXTERNAL_EFFECT",
        "plan PRECHECK allowed_effects",
    )
    for key in (
        "provider_calls", "quota_reservations", "aws_calls", "sts_calls", "s3_calls",
        "s3_get_object_version_calls", "s3_bucket_metadata_calls", "runtime_credential_accesses",
...
def validate_manifest_document(
    ...
    *,
    ...
    _require(isinstance(files_for_tests.get("live_session_policy"), Mapping),
             "INVALID_MANIFEST", "manifest.files.live_session_policy is required")
    ...
    for role, binding in files.items():
        ...
        if role == "live_session_policy" and live_session_policy_path is not None:
    ...
    live_policy_binding = files.get("live_session_policy")
    ...
    _require(isinstance(live_policy_repo_path, str) and live_policy_repo_path,
             "INVALID_MANIFEST_PATH", "manifest.files.live_session_policy.path")
    resolved_live_policy = (
        live_session_policy_path.resolve()
        if live_session_policy_path is not None
        else (repo_root / live_policy_repo_path).resolve()
    )
    validate_live_session_policy_for_aws(resolved_live_policy)
...
def _validate_zero_precheck_receipt(receipt: Mapping[str, Any]) -> str:
    ...
    match = re.fullmatch(
        r"M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_"
    ...
    _require_exact(
        ...
        "TERMINAL_PASS_FOCUSED_G11C3_PRECHECK_ZERO_EXTERNAL_EFFECT_LIVE_NOT_AUTHORIZED",
    ...
    for key, value in observed.items():
        if key not in {"effects_reconciled", "ambiguous_side_effects"}:
            _require_exact(value, 0, f"precheck observed_effects.{key}")
    ...
    for key in (
        ...
        "repository_mutations_by_workflow", "remote_custody_mutations", "aws_calls", "s3_calls",
    ):
...
def validate_runtime_live_head_binding(
    ...
    *,
    ...
    head_sha = env.get("G11C3_LIVE_HEAD_SHA", "")
    tree_sha = env.get("G11C3_LIVE_HEAD_TREE", "")
    _require(isinstance(head_sha, str) and HEX40.fullmatch(head_sha) is not None,
             "LIVE_RUNTIME_HEAD_INVALID", "G11C3_LIVE_HEAD_SHA")
    _require(isinstance(tree_sha, str) and HEX40.fullmatch(tree_sha) is not None,
             "LIVE_RUNTIME_TREE_INVALID", "G11C3_LIVE_HEAD_TREE")
    _require_exact(env.get("GITHUB_SHA"), head_sha, "GITHUB_SHA/G11C3_LIVE_HEAD_SHA")
...
def validate_live_activation_and_receipt(
    *,
    ...
    """Runner-side, zero-effect gate performed before importing executable adapter code."""
    ...
    _require_exact(activation.get("artifact"),
                   "M3TOP3_FINANCE_CA_PAGE100_G11C3_ELIGIBLE_SUCCESSOR_LIVE_ACTIVATION_v1.0",
...
def _normalize_and_validate_live_result(
    ...
    *,
    ...
    _require_exact(output.get("schema"),
                   "M3TOP3_FINANCE_CA_PAGE100_G11C3_LIVE_ENTRY_RESULT_v1.0",
    ...
    _require_exact(effects["aws_calls"],
                   effects["s3_get_calls"] + effects["s3_put_calls"] + effects["s3_other_calls"],
                   "effects.aws_calls")
    _require_exact(effects["s3_calls"], effects["aws_calls"], "effects.s3_calls")
...
def _pre_entry_live_failure(code: str, detail: str = "") -> dict[str, Any]:
    return {
        ...
        "effects": dict(LIVE_ZERO_EFFECTS),
...
def _precheck(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    return 0, {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C3_FOCUSED_PRECHECK_RESULT_v1.0",
        "verdict": "PASS",
        ...
        "observations": {
            ...
            "active_prefix_rejection_regressions": ["G11", "G11C1", "G11C2"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.mode == "live":
            ...
            result = {
                "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C3_GATE_FAILURE_v1.0",
                "verdict": "FAIL_CLOSED",
