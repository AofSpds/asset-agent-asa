"""Focused, zero-network tests for the G11 append-only LIVE adapter."""
...
ADAPTER_PATH = Path(__file__).resolve().parents[1] / "finance_page100_g11_live_adapter.py"
SPEC = importlib.util.spec_from_file_location("g11_live_adapter_under_test", ADAPTER_PATH)
...
class FakeStore:
    ...
    def _write(
        ...
        if self.fail_raw_ambiguous and key.startswith(g11.G11_RAW_PREFIX):
            raise g11.AmbiguousSideEffectError("S3_WRITE_EFFECT_AMBIGUOUS")
        if key == g11.G11_TERMINAL_RECEIPT_KEY and self.terminal_error is not None:
            raise self.terminal_error
        if (
            self.fail_block_checkpoint_ambiguous
            and key == g11.G11_CHECKPOINT_KEY
...
def test_success_starts_page_five_after_exact_reads_and_namespace_gate() -> None:
    ...
    assert result["checkpoint_binding"]["key"] == g11.G11_CHECKPOINT_KEY
    ...
    assert terminal["object"]["key"] == g11.G11_TERMINAL_RECEIPT_KEY
    assert terminal["object"]["sha256"] == g11.sha256_bytes(
        store.current[g11.G11_TERMINAL_RECEIPT_KEY].body
    )
...
def test_future_selector_is_raw_custodied_then_stops_without_cursor_advance() -> None:
    ...
    assert any(
        kind == "create_once" and key.startswith(g11.G11_RAW_PREFIX)
...
def test_raw_write_ceiling_stops_retry_before_second_provider_call() -> None:
    ...
    assert exit_code == 2
    assert result["error"]["code"] == "G11_RAW_WRITE_CEILING"
...
def test_ambiguous_raw_write_marks_effect_reconciliation_incomplete() -> None:
    ...
    assert store.actions[-1][1].startswith(g11.G11_RAW_PREFIX)
...
def test_known_terminal_put_failure_is_attempted_only_once() -> None:
    ...
    terminal_actions = [
        action for action in store.actions if action[1] == g11.G11_TERMINAL_RECEIPT_KEY
    ]
    assert exit_code == 2
    assert result["error"]["code"] == "S3_CONDITIONAL_CREATE_FAILED"
...
def test_ambiguous_terminal_put_causes_no_follow_on_mutation_or_retry() -> None:
    ...
    terminal_positions = [
        ...
        if action[1] == g11.G11_TERMINAL_RECEIPT_KEY
    ]
    assert exit_code == 2
...
def test_ambiguous_block_checkpoint_stops_before_terminal_receipt() -> None:
    ...
    assert result["effect_reconciliation"]["complete"] is False
    assert result["effects"]["terminal_receipt_put_attempts"] == 0
    assert all(key != g11.G11_TERMINAL_RECEIPT_KEY for _, key in store.actions)
    assert store.actions[-1][1] == g11.G11_CHECKPOINT_KEY


def test_public_runner_interface_is_stable() -> None:
    entrypoint = g11.build_live_adapter()
    assert g11.ADAPTER_INTERFACE_VERSION == (
        "M3TOP3_FINANCE_CA_PAGE100_G11_LIVE_ADAPTER_v1.0"
    )
    assert g11.FACTORY_SYMBOL == "create_sealed_g11_custody_adapter"
...
def test_namespace_list_gate_requires_exact_empty_nontruncated_shape() -> None:
    ...
    assert seen == [g11.G11_RAW_PREFIX, g11.G11_CONTROL_PREFIX, g11.EXECUTION_CLAIM_KEY]
    assert valid_store.api_calls == {"get": 0, "put": 0, "other": 3}

    malformed_payloads = [
        {},
        {"Prefix": g11.G11_RAW_PREFIX, "MaxKeys": 2,
         "Versions": [], "DeleteMarkers": []},
        {**valid_payload(g11.G11_RAW_PREFIX), "Prefix": "wrong/"},
        {**valid_payload(g11.G11_RAW_PREFIX), "IsTruncated": "false"},
        {**valid_payload(g11.G11_RAW_PREFIX), "IsTruncated": True},
        {**valid_payload(g11.G11_RAW_PREFIX), "Versions": [{}]},
        {**valid_payload(g11.G11_RAW_PREFIX), "DeleteMarkers": [{}]},
    ]
    for payload in malformed_payloads:
        ...
        try:
            ...
            assert exc.code == "FRESH_G11_IDENTITY_ALREADY_CONSUMED"
        else:
