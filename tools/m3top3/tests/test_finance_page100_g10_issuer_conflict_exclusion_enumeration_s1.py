#!/usr/bin/env python3
"""Focused offline tests for the exact-four-version S1 enumeration."""
...
from tools.m3top3 import (
    finance_page100_g10_issuer_conflict_exclusion_enumeration_s1 as subject,
)
...
class S1EnumerationTests(unittest.TestCase):
    def test_authority_and_compact_policy_are_exact(self) -> None:
        ...
        self.assertEqual(len(subject.canonical_json_bytes(policy)), 2046)
        changed = copy.deepcopy(policy)
        changed["Statement"][0]["Action"] = "s3:ListBucketVersions"
        with self.assertRaisesRegex(subject.S1EnumerationError, "POLICY_NOT_EXACT"):
    ...
    def test_activation_schema_freshness_and_exact_latch(self) -> None:
        for mode in ("PRECHECK", "LIVE"):
            ...
            with self.assertRaisesRegex(subject.S1EnumerationError, "NOT_FRESH"):
                subject.validate_activation(
                    mode,
                    stale,
                    b"authority\n",
        ...
        with self.assertRaisesRegex(subject.S1EnumerationError, "FRESH_RUNTIME"):
            subject.validate_activation(
                "LIVE",
                bad,
                b"authority\n",
        ...
        with self.assertRaisesRegex(subject.S1EnumerationError, "IDENTITY_INVALID"):
            subject.validate_activation(
                "PRECHECK",
                extra,
                b"authority\n",
    ...
    def test_event_lineage_is_enforced_for_both_modes(self) -> None:
        ...
        for mode in ("PRECHECK", "LIVE"):
            ...
            with mock.patch.dict(os.environ, env, clear=True), self.assertRaisesRegex(
                subject.S1EnumerationError, "EVENT_LINEAGE_MISMATCH"
            ):
                subject.validate_event_lineage(mode, value, PREP_COMMIT)
    ...
    def test_precheck_pass_exact_effects_activation_and_workflow_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            receipt = {
                ...
                "repository": subject.REPOSITORY,
                "state": "PASS_FRESH_BOUNDED_S1_PRECHECK_LIVE_NOT_STARTED",
                "workflow": {
                    "actor": subject.ACTOR,
            ...
            with self.assertRaisesRegex(subject.S1EnumerationError, "EFFECTS_NOT_ZERO"):
                subject.validate_precheck_pass(
                    root,
                    bad_effect,
                    live_value,
            ...
            with self.assertRaisesRegex(subject.S1EnumerationError, "WORKFLOW_INVALID"):
                subject.validate_precheck_pass(
                    root,
                    bad_provenance,
                    live_value,
    ...
    def test_fake_aws_exact_one_plus_four_success_and_no_retry(self) -> None:
        ...
        with self.assertRaisesRegex(subject.S1EnumerationError, "ACCOUNT_ID_INVALID"):
            rejected.read_all()
    ...
    def test_fifth_wrong_order_and_failed_call_stop_without_retry(self) -> None:
        ...
        with mock.patch.object(subject, "EXPECTED_RAW_VERSIONS", bindings), mock.patch.object(
            ...
            reader.read_all()
            with self.assertRaisesRegex(subject.S1EnumerationError, "BUDGET_EXCEEDED"):
            ...
            with tempfile.TemporaryDirectory() as name:
                with self.assertRaisesRegex(subject.S1EnumerationError, "NOT_SEQUENTIAL"):
            ...
            with self.assertRaisesRegex(subject.S1EnumerationError, "AWS_CLI_CALL_FAILED"):
                failed.read_all()
        ...
        with mock.patch.object(subject, "EXPECTED_RAW_VERSIONS", bindings), mock.patch.object(
            ...
            for mode in ("missing", "extra"):
                with self.subTest(mode=mode), self.assertRaisesRegex(
                    subject.S1EnumerationError, "RAW_USER_METADATA_EXACT_SET"
                ):
                    subject.ExactAwsCli(
