"""Focused, zero-network tests for the G11C9 append-only LIVE adapter."""
...
ADAPTER_PATH = Path(__file__).resolve().parents[1] / "finance_page100_g11c9_live_adapter.py"
SPEC = importlib.util.spec_from_file_location("g11c9_live_adapter_under_test", ADAPTER_PATH)
...
class FakeStore:
    ...
    def _write(
        ...
        if self.fail_raw_ambiguous and key.startswith(g11.G11C9_RAW_PREFIX):
            raise g11.AmbiguousSideEffectError("S3_WRITE_EFFECT_AMBIGUOUS")
        if key == g11.G11C9_TERMINAL_RECEIPT_KEY and self.terminal_error is not None:
            raise self.terminal_error
        if (
            self.fail_block_checkpoint_ambiguous
            and key == g11.G11C9_CHECKPOINT_KEY
...
def test_success_starts_page_five_after_exact_reads_and_namespace_gate() -> None:
    ...
    assert result["checkpoint_binding"]["key"] == g11.G11C9_CHECKPOINT_KEY
    ...
    assert terminal["object"]["key"] == g11.G11C9_TERMINAL_RECEIPT_KEY
    assert terminal["object"]["sha256"] == g11.sha256_bytes(
        store.current[g11.G11C9_TERMINAL_RECEIPT_KEY].body
    )
...
def test_future_selector_is_raw_custodied_then_stops_without_cursor_advance() -> None:
    ...
    assert any(
        kind == "create_once" and key.startswith(g11.G11C9_RAW_PREFIX)
...
def test_raw_write_ceiling_stops_retry_before_second_provider_call() -> None:
    ...
    assert exit_code == 2
    assert result["error"]["code"] == "G11C9_RAW_WRITE_CEILING"
...
def test_ambiguous_raw_write_marks_effect_reconciliation_incomplete() -> None:
    ...
    assert store.actions[-1][1].startswith(g11.G11C9_RAW_PREFIX)
...
def test_known_terminal_put_failure_is_attempted_only_once() -> None:
    ...
    terminal_actions = [
        action for action in store.actions if action[1] == g11.G11C9_TERMINAL_RECEIPT_KEY
    ]
    assert exit_code == 2
    assert result["error"]["code"] == "S3_CONDITIONAL_CREATE_FAILED"
    ...
    assert result["terminal_receipt_binding"] == {
        "key": g11.G11C9_TERMINAL_RECEIPT_KEY,
...
def test_ambiguous_terminal_put_causes_no_follow_on_mutation_or_retry() -> None:
    ...
    terminal_positions = [
        ...
        if action[1] == g11.G11C9_TERMINAL_RECEIPT_KEY
    ]
    assert exit_code == 2
...
def test_ambiguous_block_checkpoint_stops_before_terminal_receipt() -> None:
    ...
    assert result["effect_reconciliation"]["complete"] is False
    assert result["effects"]["terminal_receipt_put_attempts"] == 0
    assert all(key != g11.G11C9_TERMINAL_RECEIPT_KEY for _, key in store.actions)
    assert store.actions[-1][1] == g11.G11C9_CHECKPOINT_KEY


def test_public_runner_interface_is_stable() -> None:
    entrypoint = g11.build_live_adapter()
    assert g11.ADAPTER_INTERFACE_VERSION == (
        "M3TOP3_FINANCE_CA_PAGE100_G11C9_LIVE_ADAPTER_v1.0"
    )
    assert g11.FACTORY_SYMBOL == "create_sealed_g11c9_custody_adapter"
...
def test_consumed_g11_through_g11c8_are_in_successor_no_rerun_lineage() -> None:
    ...
    assert g11.OWNER_CAP_SPEC_SHA256 == "c2df5290d66beebade5c17717d41fec4d26f5f78487d23902f4fdbe72d53e31e"
    assert g11.EXECUTION_TOKEN_SHA256 == "6db13e47d9bf12142d6ac6e29f63c8ce09f198cfeabf54553462f3e6ee56f4e2"
    ...
    g11._validate_consumed_predecessor_identities(
        authority_no_rerun, "TEST_G11C7_NO_RERUN_LINEAGE_MISMATCH"
    )
    g11._validate_predecessor_terminal_g11c7_binding(
        authority, "TEST_G11C7_TERMINAL_BINDING_MISMATCH"
    )
    g11._validate_predecessor_invalidated_g11c8_binding(
        authority, "TEST_G11C8_INVALIDATION_BINDING_MISMATCH"
    )
    ...
    assert authority_no_rerun["g11c7_precheck_run_id"] == 33490803554
    assert authority_no_rerun["g11c7_live_run_id"] == 33492771321
    assert authority_no_rerun["g11c7_live_job_id"] == 99807892677
    assert authority_no_rerun["g11c7_live_credentials_issued"] == 0
    assert authority_no_rerun["g11c7_live_runner_started"] is False
    assert authority_no_rerun["consumed_g11c8_precheck_run"] == 33498757471
    assert authority_no_rerun["g11c8_precheck_run_id"] == 33498757471
    assert authority_no_rerun["g11c8_precheck_job_id"] == 99826920605
    assert authority_no_rerun["g11c8_precheck_receipt_contract_valid"] is True
    assert authority_no_rerun["g11c8_live_run_exists"] is False
    altered = copy.deepcopy(authority)
    altered["predecessor_invalidated_g11c8_binding"]["terminal_receipt_bytes"] += 1
    try:
        g11._validate_predecessor_invalidated_g11c8_binding(
            altered, "TEST_G11C8_INVALIDATION_BINDING_MISMATCH"
        )
    except g11.GovernanceError as exc:
        assert exc.code == "TEST_G11C8_INVALIDATION_BINDING_MISMATCH"
    else:
        raise AssertionError("altered G11C8 invalidation binding must fail closed")
    assert g11.ACTIVATION_BASE_HEAD_COMMIT == (
        "39a674ac8fc2d6af25e23f533f9f3379f81e4b6c"
    )
    assert g11.ACTIVATION_BASE_TREE == "9f3905bd6aa25617c5b2f93e137a9ac281c3dc7b"
...
def test_live_session_policy_exact_semantics_ascii_and_size_gate() -> None:
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        try:
            ...
            raise AssertionError("Version-bearing G11C9 policy must fail closed")
...
def test_namespace_list_gate_requires_exact_empty_nontruncated_shape() -> None:
    ...
    assert seen == [g11.G11C9_RAW_PREFIX, g11.G11C9_CONTROL_PREFIX, g11.EXECUTION_CLAIM_KEY]
    assert valid_store.api_calls == {"get": 0, "put": 0, "other": 3}

    malformed_payloads = [
        {},
        {"Prefix": g11.G11C9_RAW_PREFIX, "MaxKeys": 2,
         "Versions": [], "DeleteMarkers": []},
        {**valid_payload(g11.G11C9_RAW_PREFIX), "Prefix": "wrong/"},
        {**valid_payload(g11.G11C9_RAW_PREFIX), "IsTruncated": "false"},
        {**valid_payload(g11.G11C9_RAW_PREFIX), "IsTruncated": True},
        {**valid_payload(g11.G11C9_RAW_PREFIX), "Versions": [{}]},
        {**valid_payload(g11.G11C9_RAW_PREFIX), "DeleteMarkers": [{}]},
    ]
    for payload in malformed_payloads:
        ...
        try:
            ...
            assert exc.code == "FRESH_G11C9_IDENTITY_ALREADY_CONSUMED"
        else:
            raise AssertionError("malformed namespace listing must fail closed")
        assert store.api_calls == {"get": 0, "put": 0, "other": 1}


def test_authority_active_prefix_gate_rejects_g11_through_g11c8() -> None:
    g11._validate_active_c9_prefixes(g11.G11C9_RAW_PREFIX, g11.G11C9_CONTROL_PREFIX)
    for historical in ("G11", "G11C1", "G11C2", "G11C3", "G11C4", "G11C5", "G11C6", "G11C7", "G11C8"):
        old_raw = g11.G11C9_RAW_PREFIX.replace("G11C9", historical)
        old_control = g11.G11C9_CONTROL_PREFIX.replace("G11C9", historical)
        try:
            g11._validate_active_c9_prefixes(old_raw, g11.G11C9_CONTROL_PREFIX)
        except g11.GovernanceError as exc:
            assert exc.code == "HISTORICAL_ACTIVE_PREFIX_FORBIDDEN"
        else:
            raise AssertionError(f"{historical} raw prefix must fail closed")
        try:
            g11._validate_active_c9_prefixes(g11.G11C9_RAW_PREFIX, old_control)
        except g11.GovernanceError as exc:
            assert exc.code == "HISTORICAL_ACTIVE_PREFIX_FORBIDDEN"
        else:
...
def test_live_credentials_require_all_nine_and_never_fall_back_to_ambient() -> None:
    ...
    try:
        ...
        assert exc.code == "G11C9_EXACT_THREE_CREDENTIAL_BUNDLES_REQUIRED"
    else:
    ...
    try:
        ...
        assert exc.code == "G11C9_CREDENTIAL_BUNDLES_MUST_BE_DISTINCT"
    else:
