#!/usr/bin/env python3
"""Fail-closed core and focused PRECHECK for the Finance Page100 G11C2 successor.
...
OWNER_CAP_SPEC_SHA256 = "edbe0966e7051bd49e0f0878875d149c0668193ba6c4602b9984fbcdc0567b3d"
EXECUTION_TOKEN_SHA256 = "c5190d38a867e7d80448e03b48d0ff98b1338d02ac70a1e8ca8481d7f49c6fc2"
...
AUTHORITY_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0.json"
)
PLAN_FILENAME = "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_PLAN_v1.0.json"
SEED_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0.json"
)
MANIFEST_FILENAME = (
    "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_MANIFEST_v1.0.json"
)

AUTHORITY_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_AUTHORITY_v1.0"
PLAN_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_PLAN_v1.0"
SEED_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_CHECKPOINT_SEED_v1.0"
MANIFEST_SCHEMA = "M3TOP3_FINANCE_CA_PAGE100_G11C2_ELIGIBLE_SUCCESSOR_MANIFEST_v1.0"

LIVE_ADAPTER_GATE_BLOCKED = "BLOCKED_MISSING_EXECUTABLE_CUSTODY_ADAPTERS"
LIVE_ADAPTER_GATE_READY = "READY"
LIVE_ADAPTER_REPO_PATH = "tools/m3top3/finance_page100_g11c2_live_adapter.py"
LIVE_ADAPTER_INTERFACE_VERSION = "M3TOP3_FINANCE_CA_PAGE100_G11C2_LIVE_ADAPTER_v1.0"
LIVE_ADAPTER_FACTORY_SYMBOL = "create_sealed_g11c2_custody_adapter"
LIVE_ADAPTER_TEST_REPO_PATH = "tools/m3top3/tests/test_finance_page100_g11c2_live_adapter.py"
...
def validate_live_session_policy_for_aws(path: Path) -> int:
    ...
    _require("Version" not in parsed, "LIVE_SESSION_POLICY_VERSION_MUST_BE_OMITTED",
             "G11C2 inline policy must use the permission-equivalent default version")
    _require("${" not in source_text, "LIVE_SESSION_POLICY_VARIABLE_FORBIDDEN",
             "G11C2 inline policy may not contain policy variables")
...
def load_sealed_live_adapter_factory(adapter_path: Path, expected_sha256: str) -> Any:
    ...
    module_name = f"_m3top3_g11c2_live_adapter_{expected_sha256}"
    try:
...
def validate_g11_object_key(key: str) -> None:
    """Reject writes outside the fresh G11C2 namespace or historical namespaces."""

    _require(isinstance(key, str) and bool(key), "INVALID_G11_OBJECT_KEY", "object key is empty")
    _require("G11C2" in key and GENERATION_TIMESTAMP in key,
             "INVALID_G11_OBJECT_KEY", "key is not bound to the fresh G11C2 generation")
    for forbidden in ("G10", "S2", "S3", "PMO-FINANCE-PAGE100-G10"):
...
def validate_authority_document(document: Mapping[str, Any]) -> None:
    ...
    validate_consumed_g11c1_identities(no_rerun, "authority.no_rerun")
...
def expected_owner_cap_spec() -> dict[str, Any]:
    return {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C2_OWNER_CAP_SPEC_v1.0",
        ...
        "g10_spent_primary_acquisitions": INHERITED_G10_ACQUISITIONS,
        "g10_spent_network_attempts": INHERITED_G10_ATTEMPTS,
        "maximum_new_g11c2_primary_acquisitions": G11_ACQUISITION_CEILING,
        "maximum_new_g11c2_network_attempts": G11_ATTEMPT_CEILING,
...
def expected_execution_token_material() -> dict[str, Any]:
    return {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C2_EXECUTION_TOKEN_MATERIAL_v1.0",
        "generation_id": GENERATION_ID,
        "runtime_lock_id": RUNTIME_LOCK_ID,
        "pilot_run_id": PILOT_RUN_ID,
        "precheck_act_id": PRECHECK_ACT_ID,
...
def validate_plan_document(document: Mapping[str, Any]) -> None:
    ...
    validate_consumed_g11c1_identities(plan_no_rerun, "plan.no_rerun")
...
def validate_seed_document(document: Mapping[str, Any]) -> str:
    ...
    validate_consumed_g11c1_identities(seed_no_rerun, "seed.no_rerun")
...
def _validate_zero_precheck_receipt(receipt: Mapping[str, Any]) -> str:
    ...
    _require_exact(
        ...
        "TERMINAL_PASS_FOCUSED_G11C2_PRECHECK_ZERO_EXTERNAL_EFFECT_LIVE_NOT_AUTHORIZED",
...
def validate_runtime_live_head_binding(
    ...
    *,
    ...
    head_sha = env.get("G11C2_LIVE_HEAD_SHA", "")
    tree_sha = env.get("G11C2_LIVE_HEAD_TREE", "")
    _require(isinstance(head_sha, str) and HEX40.fullmatch(head_sha) is not None,
             "LIVE_RUNTIME_HEAD_INVALID", "G11C2_LIVE_HEAD_SHA")
    _require(isinstance(tree_sha, str) and HEX40.fullmatch(tree_sha) is not None,
             "LIVE_RUNTIME_TREE_INVALID", "G11C2_LIVE_HEAD_TREE")
    _require_exact(env.get("GITHUB_SHA"), head_sha, "GITHUB_SHA/G11C2_LIVE_HEAD_SHA")
...
def validate_live_activation_and_receipt(
    *,
    ...
    validate_consumed_g11c1_identities(no_rerun, "live activation no_rerun")
    return {
...
def _normalize_and_validate_live_result(
    ...
    *,
    ...
    _require_exact(output.get("schema"),
                   "M3TOP3_FINANCE_CA_PAGE100_G11C2_LIVE_ENTRY_RESULT_v1.0",
...
def _precheck(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    return 0, {
        "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C2_FOCUSED_PRECHECK_RESULT_v1.0",
        "verdict": "PASS",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.mode == "live":
            ...
            result = {
                "schema": "M3TOP3_FINANCE_CA_PAGE100_G11C2_GATE_FAILURE_v1.0",
                "verdict": "FAIL_CLOSED",
