...
from .core import atomic_write_json, deterministic_id, sha256_hex
...
class ValidationRunner:
    def __init__(self,scorer:ModelScorer,ranking:RankingEngine,outcome_builder:OutcomeBuilder,metrics:MetricsEngine|None=None):
        self.scorer=scorer; self.ranking=ranking; self.outcome_builder=outcome_builder; self.metrics=metrics or MetricsEngine()
    ...
    def run_snapshot(self,snapshot_dir:str|Path,output_dir:str|Path,prediction_ledger:PredictionLedger|None=None)->dict[str,Any]:
        snapshot_dir=Path(snapshot_dir); output_dir=Path(output_dir); output_dir.mkdir(parents=True,exist_ok=True)
        manifest=json.loads((snapshot_dir/"manifest.json").read_text(encoding="utf-8")); inputs=[json.loads(line) for line in (snapshot_dir/"model_input.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
        ...
        for r in selected:
            ...
            if prediction_ledger is not None: prediction_ledger.append(PredictionLedger.build_record(r,manifest["snapshot_cutoff_at"],sha256_hex(inp),status="EXPERIMENTAL"))
            ...
            outcomes.append(d)
        run_id=deterministic_id("validationrun",{"snapshot_content_hash":manifest["snapshot_content_hash"],"model_version":self.scorer.model_version,"price_dataset_id":self.outcome_builder.price.dataset_id,"validation_protocol":self.outcome_builder.validation_protocol_version})
        ...
        atomic_write_json(output_dir/f"{manifest['snapshot_date']}.json",result); return result
