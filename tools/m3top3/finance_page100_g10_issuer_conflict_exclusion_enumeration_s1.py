#!/usr/bin/env python3
"""Fresh bounded S1 issuer-group exclusion enumeration.
...
GENERATION_STAMP = "20260831130000"
GENERATION_ID = "G10-ISSUER-GROUP-EXCLUSION-ENUMERATION-S1-" + GENERATION_STAMP
RUNTIME_LOCK_ID = "PMO-G10-ISSUER-GROUP-EXCLUSION-ENUMERATION-S1-" + GENERATION_STAMP
PRECHECK_ACT_ID = (
    "G10-ISSUER-GROUP-EXCLUSION-ENUMERATION-S1-PRECHECK-" + GENERATION_STAMP
)
LIVE_ACT_ID = "G10-ISSUER-GROUP-EXCLUSION-ENUMERATION-S1-LIVE-" + GENERATION_STAMP

PREDECESSOR_TERMINAL_COMMIT = "9708c21fee0137261926417561fd4e7694571660"
PREDECESSOR_TERMINAL_TREE = "1a9da9beffa9f1d841a94b8fb945a11125025127"
PREDECESSOR_TERMINAL_PATH = (
    ...
    "ENUMERATION_PRECHECK_TERMINAL_RECEIPT_33389710172_v1.0.json"
)
PREDECESSOR_TERMINAL_BYTES = 2620
PREDECESSOR_TERMINAL_SHA256 = (
    "4c8662f08e2733fd2201aafba477aeb4019692506ea633932b7052aa53de2e3e"
)
PREDECESSOR_TERMINAL_BLOB = "037a1f7fa9e291434fee355e3634a063c95814c6"
...
AUTHORITY_PATH = (
    ...
    "REMEDIATION_ENUMERATION_S1_AUTHORITY_v1.0.json"
)
MANIFEST_PATH = (
    f"{CONTROL_ROOT}/M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_"
    "REMEDIATION_ENUMERATION_S1_MANIFEST_v1.0.json"
)
POLICY_PATH = (
    f"{CONTROL_ROOT}/M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_"
    "REMEDIATION_ENUMERATION_S1_LIVE_SESSION_POLICY_v1.0.json"
)
PRECHECK_ACTIVATION_PATH = (
    f"{CONTROL_ROOT}/M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_"
    "REMEDIATION_ENUMERATION_S1_PRECHECK_ACTIVATION_v1.0.json"
)
LIVE_ACTIVATION_PATH = (
    f"{CONTROL_ROOT}/M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_"
    "REMEDIATION_ENUMERATION_S1_LIVE_ACTIVATION_v1.0.json"
)
PRECHECK_PASS_PATH = (
    f"{CONTROL_ROOT}/M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_"
    "REMEDIATION_ENUMERATION_S1_PRECHECK_PASS_RECEIPT_v1.0.json"
)
...
PRECHECK_WORKFLOW_PATH = (
    ...
    "remediation-enumeration-s1-precheck-v1.yml"
)
LIVE_WORKFLOW_PATH = (
    ".github/workflows/m3top3-finance-page100-g10-issuer-conflict-exclusion-"
    "remediation-enumeration-s1-live-v1.yml"
)
RUNNER_PATH = (
    "tools/m3top3/"
    "finance_page100_g10_issuer_conflict_exclusion_enumeration_s1.py"
)
TEST_PATH = (
    "tools/m3top3/tests/"
    "test_finance_page100_g10_issuer_conflict_exclusion_enumeration_s1.py"
)

PREPARATION_MESSAGE = (
    "Prepare G10 issuer-group exclusion enumeration S1 20260831130000 v1.0"
)
PRECHECK_MESSAGE = (
    "Arm G10 issuer-group exclusion enumeration S1 PRECHECK 20260831130000 v1.0"
)
LIVE_MESSAGE = (
    "Arm G10 issuer-group exclusion enumeration S1 LIVE 20260831130000 v1.0"
)

AUTHORITY_ARTIFACT = (
    "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_REMEDIATION_"
    "ENUMERATION_S1_AUTHORITY_v1.0"
)
MANIFEST_ARTIFACT = (
    "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_REMEDIATION_"
    "ENUMERATION_S1_MANIFEST_v1.0"
)
PRECHECK_ACTIVATION_ARTIFACT = (
    "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_REMEDIATION_"
    "ENUMERATION_S1_PRECHECK_ACTIVATION_v1.0"
)
LIVE_ACTIVATION_ARTIFACT = (
    "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_REMEDIATION_"
    "ENUMERATION_S1_LIVE_ACTIVATION_v1.0"
)
PRECHECK_PASS_ARTIFACT = (
    "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_CONFLICT_EXCLUSION_REMEDIATION_"
    "ENUMERATION_S1_PRECHECK_PASS_RECEIPT_v1.0"
)
...
def load_canonical_json(path: pathlib.Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise S1EnumerationError("JSON_PARSE_INVALID") from exc
...
def _real_utc_timestamp(value: Any, code: str) -> dt.datetime:
    ...
    try:
        ...
        raise S1EnumerationError(code) from exc
    return parsed
...
def validate_predecessor_terminal(root: pathlib.Path) -> dict[str, Any]:
    ...
    require(
        receipt.get("state")
        == "TERMINAL_PRECHECK_PASS_ZERO_EXTERNAL_EFFECT_ENUMERATION_NOT_STARTED"
        and receipt.get("exact_enumeration_started") is False,
        "PREDECESSOR_TERMINAL_STATE_INVALID",
    )
    defect = receipt.get("authority_defect_classification")
    require(
        isinstance(defect, Mapping)
        and defect.get("control_defect")
        == "AUTHORITY_SCOPE_ZERO_S3_READ_OVERRESTRICTION"
        and all(
            defect.get(key) is True
            for key in (
                "custody_neutral",
                "deterministic",
                "quota_neutral",
                "semantic_neutral",
                "side_effects_fully_reconciled",
            )
        ),
...
def expected_policy() -> dict[str, Any]:
    resources = [
        f"arn:aws:s3:::{BUCKET}/{row['s3_object_key']}"
...
def validate_policy(policy: Mapping[str, Any]) -> None:
    ...
    require(len(encoded) <= 2048, "LIVE_SESSION_POLICY_PLAINTEXT_TOO_LARGE")
...
def validate_authority(root: pathlib.Path, authority: Mapping[str, Any]) -> None:
    require(
        ...
        == "OWNER_STANDING_AUTHORIZED_FRESH_BOUNDED_READ_ONLY_ENUMERATION_SUCCESSOR_S1"
    ...
    require(
        ...
        and successor.get("prior_precheck_generation_reused") is False
        and successor.get("prior_s4_authority_or_session_reused") is False
        and successor.get("prior_s4_latch_reused") is False,
...
def validate_manifest(
    ...
    require(
        ...
        and manifest.get("state") == "IMMUTABLE_S1_PREPARATION_MANIFEST"
...
def validate_activation(
    ...
    *,
    ...
    state = (
        "ARMED_FRESH_ZERO_EXTERNAL_EFFECT_S1_PRECHECK_ONCE"
        if mode == "PRECHECK"
    ...
    expected_activation_prefix = (
        "G10-ISSUER-GROUP-EXCLUSION-ENUMERATION-S1-PRECHECK-ACTIVATION-"
        if mode == "PRECHECK"
        else "G10-ISSUER-GROUP-EXCLUSION-ENUMERATION-S1-LIVE-ACTIVATION-"
    )
    require(
    ...
    common_fresh = {
        ...
        "prior_precheck_generation_reused": False,
...
def validate_precheck_pass(
    ...
    require(
        ...
        and receipt.get("state") == "PASS_FRESH_BOUNDED_S1_PRECHECK_LIVE_NOT_STARTED"
...
class ExactAwsCli:
    ...
    def _invoke(self, name: str, args: Sequence[str]) -> dict[str, Any]:
        ...
        try:
            ...
            raise S1EnumerationError("AWS_JSON_INVALID") from exc
    ...
    def read_all(self) -> list[bytes]:
        ...
        bodies: list[bytes] = []
        total = 0
        with tempfile.TemporaryDirectory(prefix="g10-exclusion-s1-") as temp_name:
...
def build_live_output(
    ...
    result = {
        "artifact": (
            ...
            "ENUMERATION_S1_HASH_ONLY_RESULT_v1.0"
        ),
...
def command_precheck(args: argparse.Namespace) -> int:
    ...
    output = {
        "artifact": (
            "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_GROUP_EXCLUSION_"
            "ENUMERATION_S1_PRECHECK_RESULT_v1.0"
        ),
...
def command_live_preflight(args: argparse.Namespace) -> int:
    ...
    output = {
        "artifact": (
            "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_GROUP_EXCLUSION_"
            "ENUMERATION_S1_LIVE_PREFLIGHT_RESULT_v1.0"
        ),
        "effects": dict(EXPECTED_PRECHECK_EFFECTS),
        "generation_id": GENERATION_ID,
...
def main() -> int:
    ...
    except (S1EnumerationError, pure.ExclusionPrecheckError) as exc:
        failure = {
            "artifact": "M3TOP3_FINANCE_CA_PAGE100_G10_ISSUER_GROUP_EXCLUSION_ENUMERATION_S1_FAILURE_v1.0",
            "code": str(exc),
            "state": "FAIL_CLOSED",
        }
